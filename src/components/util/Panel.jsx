...
export const Panel = ({children,onClick,topImg,bottomImg})=>{
    const [mouseHovered,setMouseHovered] = useState(false);
    return <div onMouseOver={()=>setMouseHovered(true)}
     onMouseLeave={()=>setMouseHovered(false)} 
     onClick={onClick} 
     className={`relative dashboard-item border-2 flex flex-col justify-center border-slate-200 rounded-lg shadow-lg py-4 ${mouseHovered? 'text-white' : 'text-slate-900'} duration-150 ease-in hover:scale-105 cursor-pointer select-none overflow-hidden`}>
